...
```{r Database Header, results = 'asis', eval = params$include_enrichment}
cat("### Database", "\n")
cat("Which gene set/pathway database was used for enrichment analysis.", "\n")
```

```{r Database, eval = params$include_enrichment}
...
if (params$enrichment == "ranked"){
  ...
  for (i in 1:length(params$enriched)){
    if (dim(params$enriched[[i]][[1]])[1] != 0){
      dot <- enrichedPlots(enriched = params$enriched[[i]][[1]],
                           gmt = params$enrichment_database,
                           geneList = params$geneList[[i]][[1]],
                           contrast = '',
                           plottype = "dot",
...
suppressWarnings({
  if (params$enrichment == "ranked"){
    ...
    for (i in 1:length(params$enriched)){
      if (dim(params$enriched[[i]][[j]])[1] != 0){
        cnet <- enrichedPlots(enriched = params$enriched[[i]][[1]],
                           gmt = params$enrichment_database,
                           geneList = params$geneList[[i]][[1]],
                           contrast = '',
                           plottype = "cnet",
...
suppressWarnings({
  if (params$enrichment == "ranked") {
    ...
    for (i in 1:length(params$enriched)){
      if (dim(params$enriched[[i]][[j]])[1] != 0){
        emap <- enrichedPlots(enriched = params$enriched[[i]][[1]],
                           gmt = params$enrichment_database,
                           geneList = params$geneList[[i]][[1]],
                           contrast = '',
                           plottype = "emap",
...
if (params$enrichment == "ranked") {
  ...
  for (i in 1:length(params$enriched)){
    if (dim(params$enriched[[i]][[j]])[1] != 0){
      upset <- enrichedPlots(enriched = params$enriched[[i]][[1]],
                    gmt = params$enrichment_database,
                    geneList = params$geneList[[i]][[1]],
                    contrast = '',
                    plottype = "upset",
...
suppressWarnings({
  if (params$enrichment == "ranked") {
    ...
    for (i in 1:length(params$enriched)){
      if (dim(params$enriched[[i]][[j]])[1] != 0){
        tree <- enrichedPlots(enriched = params$enriched[[i]][[1]],
                        gmt = params$enrichment_database,
                        geneList = params$geneList[[i]][[1]],
                        contrast = '',
                        plottype = "tree",
...
if (params$enrichment == "ranked") {
  for (i in 1:length(params$enriched)){
    for (j in 1:length(params$enriched[[i]])){
      if (!is.character(params$enriched[[i]][[j]])){
        ...
        if (dir.exists(save_dir)){
          ...
          algo <- sub(paste0(dataset, "_"), "", names(params$geneList)[i])
          
          if (algo == "GSEA"){
            write_delim(df, paste0(save_dir, "/", dataset, "/Enrichment/CytoScapeInputs/", params$contrasts[j], "_", params$enrichment_database[1], "_enrichment.gmt.txt"), delim = "\t")
          } else if (algo == "PKSEA"){
            write_delim(df, paste0(save_dir, "/", dataset, "/Enrichment/CytoScapeInputs/", params$contrasts[j], "_", params$enrichment_database[2], "_enrichment.gmt.txt"), delim = "\t")
          }
          
          write_delim(data.frame(names(params$geneList[[i]][[j]]), params$geneList[[i]][[j]]), paste0(save_dir, "/", dataset, "/Enrichment/CytoScapeInputs/ranked_", params$contrasts[j], ".rnk"), delim = "\t")
        }
      }
    }
  }
  ...
  for (i in 1:length(params$enriched)){
    if (!is.character(params$enriched[[i]][[1]])){
      df <- makeCytoscapeInput(enrichmentResult = params$enriched[[i]][[1]])
        
      if (dir.exists(save_dir)){
        dataset <- sub("_[^_]+$", "", names(params$geneList)[i])
        write_delim(df, paste0(save_dir, "/", dataset, "/Enrichment/CytoScapeInputs/", "ORA_", params$enrichment_database, "_enrichment.gmt.txt"), delim = "\t")
        write_delim(data.frame(params$geneList), paste0(save_dir, "/", dataset, "/Enrichment/CytoScapeInputs/UNranked_", params$contrasts[j], ".rnk"), delim = "\t")
      }
    }
  }
}
...
cat("File Name(s): ", paste0(params$type, collapse = "; "), "\n",
    ...
    "\t Enrichment Database: ", params$enrichment_database, "\n")
```
