...
# mac downloads directory
if (.Platform$OS.type == "unix"){
  save_dir <- paste0("~/Downloads/", params$file_name)
} else if (.Platform$OS.type == "windows"){
  save_dir <- paste0("C:/Users/", Sys.getenv("USERNAME"), "/Downloads/", params$file_name)
}
dir.create(file.path(save_dir), showWarnings = FALSE)

print(file.path(save_dir))

for (i in 1:length(params$type)){
  dir.create(file.path(paste0(save_dir, "/", params$type[i], sep = "")), showWarnings = FALSE)
}

# # Temporary folder for saving outputs on the server
# save_dir <- file.path(tempdir(), params$file_name)
# dir.create(save_dir, showWarnings = FALSE)
# 
# print(save_dir)  # this is now a server path
# 
# # Create subfolders
# for (i in seq_along(params$type)) {
#   dir.create(file.path(save_dir, params$type[i]), showWarnings = FALSE)
# }
```
...
```{r Download Excel Summary, eval = params$has_linear_model}
...
proj_name <- ifelse((params$project_name != ""), paste0(params$project_name, "_"), params$project_name)
openxlsx::saveWorkbook(wbOut, file = paste0(save_dir, "/", proj_name, "OmNI_DataSummary.xlsx"))
```
...
```{r Model Equation}
if (params$has_linear_model) {
  print(params$model_equation)
} else {
  print("No linear model generated.")
}
```
...
for (i in 1:length(params$eset)){
  ...
  cat(params$type[i], ": \n",
      ...
      "\t Number of Missing Features in Processed Data:", sum(is.na(exprs_eset)), "\n")
  cat("\n\n")
}
```
...
for (i in 1:length(params$eset)){
  UMAP <- drawUMAP(eset = params$eset[[i]], 
                    ...
                    add_labels = TRUE,
                    shapes = FALSE,
                    use_size = length(Biobase::pData(params$eset[[i]])$Group))
  plot(UMAP)
}
```
...
for (i in 1:length(params$batch_pca_data)){
  for (j in 1:length(params$batch_pca_data[[i]])){
    ...
    eset = params$eset[[i]]
    try({pData(eset)$Group <- pData(params$batch_pca_data[[i]][[j]])$Group})
    pre_correction <- drawPCA(eset = eset,
                              x_axis = "PC1",
                              y_axis = "PC2",
                              type = params$type[[i]],
                              color = "Paired",
...
```{r Volcano Header, results = 'asis', eval = params$has_linear_model}
cat("# Volcano Plots")
```

```{r Volcano Description, results = 'asis', eval = params$has_linear_model}
...
```{r Volcano Adj P-Val SubHeader, results = 'asis', eval = params$has_linear_model}
cat("### Adjusted P-Value")
```

```{r Volcano Plot Adj P-Val, fig.height = 7, fig.width = 10, eval = params$has_linear_model}
for (i in 1:length(params$eset)){
...
```{r Volcano P-Val SubHeader, results = 'asis', eval = params$has_linear_model}
cat("### P-Value")
```

```{r Volcano Plot P-Val, fig.height = 7, fig.width = 10, eval = params$has_linear_model}
for (i in 1:length(params$eset)){
  for (item in params$contrasts){
    table <- makeTopTable(fit = params$linear_model[[i]],
                          coef = item,
...
```{r Volcano Download, include = FALSE, fig.height = 7, fig.width = 10, eval = params$has_linear_model}
for (i in 1:length(params$eset)){
  for (item in params$contrasts){
    table <- makeTopTable(fit = params$linear_model[[i]],
                          coef = item,
...
```{r Diff Heatmaps Header, results = 'asis', eval = params$has_linear_model}
cat("# Differential Heatmaps")
```

```{r Diff Heatmaps Description, results = 'asis', eval = params$has_linear_model}
...
```{r Differential Heatmaps, fig.height = 10, fig.width = 10, eval = params$has_linear_model}
if (params$rendered_by_shiny) {
...
if (params$enrichment == "ranked"){
  for (i in 1:length(params$enriched)){
    for (j in 1:length(params$enriched[[i]])){
      cat(names(params$enriched)[i], names(params$enriched[[i]])[j], ":", (dim(params$enriched[[i]][[j]])[1] != 0), "\n")
    }
  }
}
```
...
if (params$enrichment == "ranked"){
    ...
    for (j in 1:length(params$enriched[[i]])){
      if (dim(params$enriched[[i]][[j]])[1] != 0){
        dot <- enrichedPlots(enriched = params$enriched[[i]][[j]],
                             contrast = sub(paste0(params$type, "_", collapse = "|"), "", names(params$enriched)[i]),
                             plottype = "dot",
                             dotplot_title = paste(sub(paste0("_", params$contrasts, collapse = "|"), "", names(params$enriched)[i]), names(params$enriched[[i]])[j], "Dot Plot"),
                             dotplot_categories = 20)
        
        try(plot(dot))
        
        if (dir.exists(save_dir)){
          dataset <- sub(paste0("_", params$contrasts, collapse = "|"), "", names(params$enriched)[i])
  ...
  names <- c()
  for (i in 1:length(params$enriched)){
    if (!(sub(paste0("_", params$contrasts, collapse = "|"), "", names(params$enriched)[i]) %in% names)){
      for (j in 1:length(params$enriched[[i]])){
        if (dim(params$enriched[[i]][[j]])[1] != 0){
          dot <- enrichedPlots(enriched = params$enriched[[i]][[j]],
                               contrast = '',
                               plottype = "dot",
                               dotplot_title = paste(sub(paste0("_", params$contrasts, collapse = "|"), "", names(params$enriched)[i]), names(params$enriched[[i]])[j], "ORA Dot Plot"),
                               dotplot_categories = 20)
      ...
      for (j in 1:length(params$enriched[[i]])){
        if (dim(params$enriched[[i]][[j]])[1] != 0){
          ...
          cnet <- enrichedPlots(enriched = params$enriched[[i]][[j]],
                                ...
                                cnet_title = paste(sub(paste0("_", params$contrasts, collapse = "|"), "", names(params$enriched)[i]), names(params$enriched[[i]])[j], "Ranked Analysis Network Plot"))
        ...
        for (j in 1:length(params$enriched[[i]])){
          if (dim(params$enriched[[i]][[j]])[1] != 0){
            ...
            cnet <- enrichedPlots(enriched = params$enriched[[i]][[j]],
                                  ...
                                  cnetplot_layout = "kk",
                                  cnetplot_label = "category",
                                  cnet_title = paste(sub(paste0("_", params$contrasts, collapse = "|"), "", names(params$enriched)[i]), names(params$enriched[[i]])[j], "ORA Network Plot"))
      ...
      for (j in 1:length(params$enriched[[i]])){
        if (dim(params$enriched[[i]][[j]])[1] != 0){
          emap <- enrichedPlots(enriched = params$enriched[[i]][[j]],
                                ...
                                plottype = "emap",
                                emap_categories = 45,
                                emap_title = paste(sub(paste0("_", params$contrasts, collapse = "|"), "", names(params$enriched)[i]), names(params$enriched[[i]])[j], "Enrichment Map"),
        ...
        for (j in 1:length(params$enriched[[i]])){
          if (dim(params$enriched[[i]][[j]])[1] != 0){
            emap <- enrichedPlots(enriched = params$enriched[[i]][[j]],
                                  ...
                                  plottype = "emap",
                                  emap_categories = 45,
                                  emap_title = paste(params$enriched[i], names(params$enriched[[i]])[j], "ORA Enrichment Map"),
                                  emap_category_label = 0.7)
    ...
    for (j in 1:length(params$enriched[[i]])){
      if (dim(params$enriched[[i]][[j]])[1] != 0){
        upset <- enrichedPlots(enriched = params$enriched[[i]][[j]],
                               ...
                               plottype = "upset",
                               upset_categories = 15,
                               upsetplot_title = paste(sub(paste0("_", params$contrasts, collapse = "|"), "", names(params$enriched)[i]), names(params$enriched[[i]])[j], "UpSet Plot"))
      ...
      for (j in 1:length(params$enriched[[i]])){
        if (dim(params$enriched[[i]][[j]])[1] != 0){
          upset <- enrichedPlots(enriched = params$enriched[[i]][[j]],
                                 ...
                                 plottype = "upset",
                                 upset_categories = 15,
                                 upsetplot_title = paste(sub(paste0("_", params$contrasts, collapse = "|"), "", names(params$enriched)[i]), names(params$enriched[[i]])[j], "ORA UpSet Plot"))
      ...
      for (j in 1:length(params$enriched[[i]])){
        if (dim(params$enriched[[i]][[j]])[1] > 5){
          tree <- enrichedPlots(enriched = params$enriched[[i]][[j]],
                                ...
                                tree_categories = 30,
                                tree_clusters = 5,
                                treeplot_title = paste(sub(paste0("_", params$contrasts, collapse = "|"), "", names(params$enriched)[i]), names(params$enriched[[i]])[j], "Tree Diagram"))
        ...
        for (j in 1:length(params$enriched[[i]])){
          if (dim(params$enriched[[i]][[j]])[1] > 5){
            tree <- enrichedPlots(enriched = params$enriched[[i]][[j]],
                                  ...
                                  tree_categories = 30,
                                  tree_clusters = 5,
                                  treeplot_title = paste(sub(paste0("_", params$contrasts, collapse = "|"), "", names(params$enriched)[i]), names(params$enriched[[i]])[j], "ORA Tree Diagram"))
...
```{r Excel Enrichment Summaries openxlsx, eval = (params$include_enrichment)}
for (i in 1:length(params$enriched)){
  ...
  for (j in 1:length(params$enriched[[i]])){
    sheet <- paste0(names(params$enriched[[i]])[j])
    if (dim(params$enriched[[i]][[j]])[1] != 0){
      openxlsx::addWorksheet(wb = wb, sheetName = sheet)
      openxlsx::writeDataTable(wb = wb, sheet = sheet, x = params$enriched[[i]][[j]]@result, keepNA = FALSE, tableStyle = "none")
    } 
  }
...
if (params$enrichment == "ranked") {
  if (dir.exists(save_dir)){
    datasets <- unique(sub("_[^_]+$", "", names(params$geneList)))
    for (i in datasets){
...
```{r S-Score Table Download, results = 'asis', eval = params$include_sscore}
for (i in 1:length(params$sscore)){
  readr::write_csv(params$sscore[[i]], paste0(save_dir, "/MultiOmicIntegration/", names(params$sscore)[i], ".csv"))
}
```
...
if (dir.exists(save_dir)){
  dir.create(file.path(paste0(save_dir, "/MultiOmicIntegration/Enrichment")), showWarnings = FALSE)
}
```

```{r S-Score Enrichment Table, results = 'asis', eval = (params$include_sscore & params$include_enrichment)}
for (i in 1:length(params$sscore_enriched)){
  readr::write_csv(params$sscore_enriched[[i]]@result, paste0(save_dir, "/MultiOmicIntegration/Enrichment/", names(params$sscore_enriched)[i], "_GOBP.csv"))
}
```
...
if (dir.exists(save_dir)){
  for (i in 1:length(params$pcsf_nodes_net)){
    visNetwork::visSave(params$pcsf_nodes_net[[i]], file = paste0(save_dir, "/NetworkAnalysis/PCSF_Nodes_", names(params$pcsf_nodes_net)[i] ,".html"), selfcontained = TRUE)
  }
}
```
...
if (dir.exists(save_dir)){
  for (i in 1:length(params$pcsf_influential_net)){
    visNetwork::visSave(params$pcsf_influential_net[[i]], file = paste0(save_dir, "/NetworkAnalysis/PCSF_Influential_", names(params$pcsf_influential_net)[i] ,".html"), selfcontained = TRUE)
  }
}
```
...
```{r PCSF Enrichment CSV, results = 'asis', eval = params$include_pcsf}
if (params$include_enrichment == TRUE){
  for (i in 1:length(params$pcsf_enriched)){
    readr::write_csv(pcsfEnrichedTable(params$pcsf_enriched[[i]]), paste0(save_dir, "/NetworkAnalysis/", names(params$pcsf_enriched)[i], "_GOBP.csv"))
  }
}
```
...
if (params$include_enrichment == TRUE){
  if (dir.exists(save_dir)){
    for (i in 1:length(params$pcsf_enriched_network)){
      visNetwork::visSave(params$pcsf_enriched_network[[i]], file = paste0(save_dir, "/NetworkAnalysis/PCSF_Enriched_", names(params$pcsf_enriched_network)[i] ,".html"), selfcontained = TRUE)
    }
  }
}
```
...
```{r Glimma, eval = params$has_linear_model}
