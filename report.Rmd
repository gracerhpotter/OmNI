...
```{r Download Excel Summary, eval = (!is.null(str(params$linear_model)))}
...
```{r Model Equation}
if (!is.null(str(params$linear_model))) {
...
```{r Volcano Header, results = 'asis', eval = (!is.null(str(params$linear_model)))}
cat("# Volcano Plots")
```

```{r Volcano Description, results = 'asis', eval = (!is.null(str(params$linear_model)))}
...
```{r Volcano Adj P-Val SubHeader, results = 'asis', eval = (!is.null(str(params$linear_model)))}
cat("### Adjusted P-Value")
```

```{r Volcano Plot Adj P-Val, fig.height = 7, fig.width = 10, eval = (!is.null(str(params$linear_model)))}
for (i in 1:length(params$eset)){
...
```{r Volcano P-Val SubHeader, results = 'asis', eval = (!is.null(str(params$linear_model)))}
cat("### P-Value")
```

```{r Volcano Plot P-Val, fig.height = 7, fig.width = 10, eval = (!is.null(str(params$linear_model)))}
for (i in 1:length(params$eset)){
  for (item in params$contrasts){
    table <- makeTopTable(fit = params$linear_model[[i]],
                          coef = item,
...
```{r Volcano Download, include = FALSE, fig.height = 7, fig.width = 10, eval = (!is.null(str(params$linear_model)))}
for (i in 1:length(params$eset)){
  for (item in params$contrasts){
    table <- makeTopTable(fit = params$linear_model[[i]],
                          coef = item,
...
```{r Diff Heatmaps Header, results = 'asis', eval = (!is.null(str(params$linear_model)))}
cat("# Differential Heatmaps")
```

```{r Diff Heatmaps Description, results = 'asis', eval = (!is.null(str(params$linear_model)))}
...
```{r Differential Heatmaps, fig.height = 10, fig.width = 10, eval = (!is.null(str(params$linear_model)))}
if (params$rendered_by_shiny) {
...
```{r Glimma, eval = (!is.null(str(params$linear_model)))}
